...
installing at `C:\Helm` (changeable on the installer's directory page) and
does **not** need admin rights just to install or to open the app
afterward. Open Helm, click **Set up Helm**, then **Register services** &mdash;
that step alone prompts for admin rights once, since registering Windows
services genuinely requires it. That's it: one `.exe`, nothing else to
install first, and no npm/VS Code/terminal involved in actually using it
day to day &mdash; those are only needed to build Helm itself (see below).
...
`build/installer.nsh`) so the app never needs admin rights just to launch
or write its own data &mdash; only the in-app "Register services" action
prompts for elevation, since Windows service registration genuinely
requires it regardless of install location.
